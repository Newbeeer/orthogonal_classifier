...
def save_adult_datasets():
    if os.path.exists('fairness/data/adult_binary.pkl'):
        return

    adult_data = pd.read_csv('fairness/data/adult.data.txt', header=None, sep=', ').to_numpy()
    adult_test = pd.read_csv('fairness/data/adult.test.txt', header=None, sep=', ').to_numpy()
    ...
    with open('fairness/data/adult_binary.pkl', 'wb') as f:
        pkl.dump(adult_binary, f)
    with open('fairness/data/adult_test_binary.pkl', 'wb') as f:
        pkl.dump(adult_test_binary, f)


def create_adult_datasets(batch=64):
    with open('fairness/data/adult_binary.pkl', 'rb') as f:
        ab = pkl.load(f)
    with open('fairness/data/adult_test_binary.pkl', 'rb') as f:
        atb = pkl.load(f)
...
def create_torch_dataloader(batch=64):
    ...
    print(f"train len: {len(Adult_train)}, test len: {len(Adult_test)}")
