...
checkpoint = torch.load(os.path.join(args.output_dir,f'stage2_{args.bias[1]}_reweight'))
...
def main(epoch):
    ...
    for x, _, _, y, _ in tqdm(train_minibatches_iterator):
        ...
        x = x.to(device)
        y_ = y.cuda()
        p_partial = F.softmax(algorithm.predict(x), dim=1)[:, 1]
    ...
    np.save(os.path.join(args.output_dir,f"bias_{args.bias[1]}_data_reweight"), np.array(data_list))
    np.save(os.path.join(args.output_dir,f"bias_{args.bias[1]}_label_reweight"), np.array(label_list))
    print("The statistics are saved at {}".format(os.path.join(args.output_dir, f"bias_{args.bias[1]}_data_reweight")))
