...
if __name__ == '__main__':
    ...
    if opt.dataset in vars(dataset_domainbed):
        dataset = vars(dataset_domainbed)[opt.dataset](opt.data_dir, opt.bias, [], hparams)
    else:
    ...
    checkpoint = torch.load(f'stage3_model_{0.6}')
    algorithm_spur.load_state_dict(checkpoint['model_dict'])
    algorithm_spur.cuda()
    algorithm_spur.eval()

    algorithm_inv = algorithm_class(hparams['input_shape'], hparams['num_classes'], 1, hparams)
    checkpoint = torch.load(f'checkpoint/stage1_{0.6}')
    algorithm_inv.load_state_dict(checkpoint['model_dict'])
    algorithm_inv.cuda()
    algorithm_inv.eval()

    algorithm_full = algorithm_class(hparams['input_shape'], hparams['num_classes'], 1, hparams)
    checkpoint = torch.load(f'checkpoint/stage2_{0.6}')
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(train_loader):  # inner loop within one epoch
            ...
            model.set_input(data, net_inv=algorithm_inv, net_full=algorithm_full, net_spur=algorithm_spur)         # unpack data from dataset and apply preprocessing
