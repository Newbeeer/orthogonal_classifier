...
def main(epoch):
    ...
    for batch in train_minibatches_iterator:
        ...
        weights = F.softmax(algorithm_inv.predict(minibatches_device[0][0]), dim=1)[:, 0]
        weights = (1-weights) / weights
        # keep Q_1Q_2 intact
        weights[minibatches_device[0][1]==1] = 1
        weights = torch.clamp(weights, 0, 10)
        step_vals = algorithm.update(minibatches_device, weights=weights)
        ...
        if args.step % checkpoint_freq == 0:
            ...
            if results[name_out] > best_acc_out:
                ...
                if args.model_save:
                    ...
                    torch.save(save_dict, path) if args.save_path is None else torch.save(save_dict, os.path.join(args.output_dir,args.save_path))
