...
print("Load:", os.path.join(args.output_dir,f'stage1_0.6'))
checkpoint = torch.load(os.path.join(args.output_dir,f'stage1_0.6'))
...
def main(epoch):
    ...
    correct = 0.
    kl_sum = 0.
    for x, _, y, _, _ in tqdm(train_minibatches_iterator):
    ...
    np.save(os.path.join(args.output_dir,f"bias_{args.bias[0]}_data_z1"), np.array(data_list))
    np.save(os.path.join(args.output_dir,f"bias_{args.bias[0]}_label_z1"), np.array(label_list))
    print("The statistics are saved at {}".format(os.path.join(args.output_dir, f"bias_{args.bias[0]}_data_z1")))
