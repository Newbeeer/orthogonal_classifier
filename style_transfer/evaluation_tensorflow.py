...
def get_activations(images, sess, batch_size=128, verbose=False):
...
def load_image_batch(files):
    ...
    """
    if args.pre:
        return np.array([imread(str(os.path.join('/home/ylxu/domainbed/celeba', "img_align_celeba", fn))).astype(np.float32) for fn in files])
    else:
        return np.array([imread(str(fn)).astype(np.float32) for fn in files])

def get_activations_from_files(files, sess, batch_size=128, verbose=False):
    """Calculates the activations of the pool_3 layer for all images.
    Params:
    -- files      : list of paths to image files. Images need to have same dimensions for all files.
    -- sess        : current session
...
def calculate_activation_statistics_from_files(files, sess, batch_size=128, verbose=False):
    """Calculation of the statistics used by the FID.
    Params:
    -- files      : list of paths to image files. Images need to have same dimensions for all files.
    -- sess        : current session
...
def _handle_path(path, sess, low_profile=True):
    if path.endswith('.npz'):
        ...
        if args.pre:
            ...
            fn = partial(os.path.join, '/home/ylxu/domainbed/celeba')
            filename = pandas.read_csv(fn('file_path_' + args.name + '.csv'), header=0)['file_path'].values
            files = list(filename)
        else:
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False):
    ...
    with tf.device('/GPU:'+args.gpu):
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            m2, s2 = _handle_path(paths[0], sess, low_profile=True)
            if args.pre:
                np.save(f'/data/scratch/ylxu/model/celeba-mean-tf-{args.name}.npy', m2)
                np.save(f'/data/scratch/ylxu/model/celeba-variance-tf-{args.name}.npy',s2)
                print("Saving statistics")
                exit(0)

            print("Loading from ", f'/data/scratch/ylxu/model/celeba-mean-tf-{args.name}.npy')
            m1 = np.load(f'/data/scratch/ylxu/model/celeba-mean-tf-{args.name}.npy')
            print("Loading from ", f'/data/scratch/ylxu/model/celeba-variance-tf-{args.name}.npy')
            s1 = np.load(f'/data/scratch/ylxu/model/celeba-variance-tf-{args.name}.npy')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--inception", type=str, default='/data/scratch/ylxu/model/inception',
