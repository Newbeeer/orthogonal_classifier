...
if args.dataset in vars(dataset_domainbed):
    dataset = vars(dataset_domainbed)[args.dataset](args.data_dir)
else:
    ...
    for batch in train_minibatches_iterator:
        ...
        if args.step % checkpoint_freq == 0:
            ...
            for name, loader, weights in evals:
                acc = misc.accuracy_ori(algorithm, loader, weights, device)
            ...
            if best_acc_out < results[name_out]:
                ...
                print("New best checkpoint with acc:{}".format(best_acc_out)) if args.save_path is None else print("Save to:{}".format(args.save_path))
                if args.model_save:
                    save_dict = {
                        "args": vars(args),
                    ...
                    torch.save(save_dict, os.path.join(args.output_dir, args.save_path))
