...
print("Train the orthogonal classifier")
...
def main(epoch):
    ...
    for batch in train_minibatches_iterator:
        ...
        p_partial = r / (1+r)
        p_partial = p_partial.unsqueeze(1)
        y = torch.cat([p_partial, 1-p_partial], dim=1)
        minibatches_device = [(x.to(device), y.to(device))
                              for x, _, _, _, _ in batch]
        step_vals = algorithm_orth.update_kl(minibatches_device)
            ...
            if best_acc_out < results[name_out]:
                ...
                if args.model_save:
                    save_dict = {
                        ...
                        "model_dict": algorithm_orth.state_dict()
                    }
