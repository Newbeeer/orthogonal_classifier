...
parser.add_argument('--bias', type=float, nargs='+', default=[0.6, 0.6, 0.4])
...
if args.dataset == 'Celeba':
    if args.age:
        gnames = ['male_young_refine', 'female_young_refine', 'male_old_refine', 'female_old_refine']
        dataset = vars(dataset_domainbed)[args.dataset](gnames=gnames, image_size=args.image_size, young=True)
        args.test_envs = [0]
    else:
        gnames = ['male_nonblond_refine', 'male_blond_refine', 'female_blond_refine', 'female_nonblond_refine']
        dataset = vars(dataset_domainbed)[args.dataset](gnames=gnames, image_size=args.image_size, dg=True)
    print("Group names:", gnames)
    print("dataset len:", len(dataset))
elif args.dataset in vars(dataset_domainbed):
    dataset = vars(dataset_domainbed)[args.dataset](args.data_dir, args.bias, args.test_envs, hparams, eval=False)
else:
...
algorithm.to(device)
steps_per_epoch = max([len(env) / hparams['batch_size'] for env in in_splits])
        ...
        if args.step % checkpoint_freq == 0:
            ...
            if best_acc_out < val_acc:
                ...
                if args.model_save:
                    ...
                    if args.dataset == 'Celeba':
                        torch.save(save_dict, fn(f'stage{args.stage}_{gnames[0]}_{gnames[1]}_128_{args.algorithm}'))
                        print("Save to:", fn(f'stage{args.stage}_{gnames[0]}_{gnames[1]}_128_{args.algorithm}'))
                    else:
